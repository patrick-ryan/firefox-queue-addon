...
ss.storage.windows[0] = ["Test", [["tab1", "www.patrick--ryan.com"], ["tab2", "www.example.com"]]];
...
function overQuota() {
    // TODO: tell user window cannot be saved
    console.log("Window cannot be saved...");
}

// TODO: verify accuracy of quota usage percentage
wmPanel.port.on("save-clicked", function(title) {
...
wmPanel.port.on("open-clicked", function(tabs) {
    var win = allWindows.open({
        url: tabs[0][1],
        onOpen: function() {
            for (var i=1; i<tabs.length; i++) {
                allTabs.open(tabs[i][1]);
            }
        }
    });
});
