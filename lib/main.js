...
var tabQueueFolderId = bmsvc.getChildFolder(menuFolder, "Tab Queue");
if (tabQueueFolderId == 0) {
    tabQueueFolderId = bmsvc.createFolder(menuFolder, "Tab Queue", bmsvc.DEFAULT_INDEX);
}
var numFolders = 0;
panel.port.emit("activate-bookmarking");

panel.port.on("bookmark-clicked", function() {
    var folderName = "Queue";
    for each (var tab in queue) {
        var uri = ios.newURI(tab.url, null, null);
        if (!bmsvc.isBookmarked(uri)) {
            var newBkmkId = bmsvc.insertBookmark(tabQueueFolderId, uri, bmsvc.DEFAULT_INDEX, tab.title);
        }
    }
    panel.port.emit("done-bookmarking", folderName);
});
