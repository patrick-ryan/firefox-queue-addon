...
var { Group, save, search, MENU } = require("sdk/places/bookmarks");
...
panel.port.on("bookmark-clicked", function() {
    search({ title: "Tab Queue", group: MENU }).on("end", function(results) {
        if (results.length == 0) {
            var tabQueueGroup = Group({ title: "Tab Queue", group: MENU });
            save(tabQueueGroup).on("end", function(newResults) {
                saveBookmarks(newResults[0]);
            });
        }
        else {
            saveBookmarks(results[0]);
        }
    });
}

function saveBookmarks(tabQueueGroup) {
    var queueGroup = Group({ group: tabQueueGroup });
    save(queueGroup).on("end", function(results) {
        newQueueGroup = results[0];
        newQueueGroup.title = "Queue " + newQueueGroup.index.toString();
        var bookmarks = [];
        for each (var tab in queue) {
            bookmarks.push({ title: tab.title, url: tab.url, group: newQueueGroup });
        }
        save(bookmarks).on("end", function() {
            panel.port.emit("done-bookmarking", newQueueGroup.title);
        });
    });
}
