...
// consider using openTab instead of allTabs.open
...
// TODO: if panel hides, then cancel (remove form)
// TODO: debug saving problem (seems to only work when pressing <Enter> after entering title)
panel.port.on("title-entered", function(title) {
    console.log("Saving window...", title);
    console.log(queue);
    ss.storage.windows.push([title, queue]);
    wmPanel.show({
        position: wmButton
    });
});
